...
export default function StockModal({ isOpen, onClose, stok, onSave }: ModalProps) {
  const [jumlah, setJumlah] = useState<number>(stok?.jumlah || 0);
  const [permintaan, setPermintaan] = useState<string>(stok?.permintaan || "");

  useEffect(() => {
    if (stok) {
      setJumlah(stok.jumlah);
      setPermintaan(stok.permintaan);
    }
  }, [stok]);
    ...
    <AnimatePresence>
      {isOpen && stok && (
        ...
        >
          ...
          >
            <h2 className="text-lg font-semibold mb-2 dark:text-white">Edit Stok Barang</h2>
            <p className="mb-4 text-sm text-gray-500 dark:text-gray-300">{stok.deskripsi}</p>
            ...
            <label className="block text-sm font-medium dark:text-gray-200">Permintaan</label>
            <textarea
              value={permintaan}
