...
import { ActivityLog, LogEntry as ActivityLogEntry } from "@/components/stock/activityLog";
...
export default function StockPage() {
  ...
  // Fetch items berdasarkan sheet & search
  const fetchItems = useCallback(async () => {
    ...
    try {
      ...
      setItems(formatted);
      setTotalStok(
        formatted.reduce((sum, item) => sum + (parseInt(item.Jumlah) || 0), 0)
      );
    } catch {
  ...
  // Handler perubahan form input (input & select)
  ...
  // Auto fetch saat search/activeSheet berubah
  useEffect(() => {
    fetchItems();
  }, [searchQuery, activeSheet, fetchItems]);

  // Data chart & recent logs
  ...
  // Export handlers
  const handleExportExcel = () => exportToExcel(items, `stok-${activeSheet}`);
  const handleExportPDF = () => exportToPDF(items, `stok-${activeSheet}`);
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-6 w-full">
      ...
      <StockTable
        ...
        items={items}
        editedLot={editingLot}
        onEdit={handleEdit}
        onDelete={handleDelete}
      />

      {/* Riwayat aktivitas */}
      <ActivityLog logs={recentLogs as ActivityLogEntry[]} />
