// File: src/components/Dasboards/Dasboard_Advance/FormAdvanceModal.tsx
'use client';
...
// Pastikan tipe data ini diekspor
...
export default function AdvanceFormModal({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Gunakan methodOverride untuk Apps Script
      if (isEditMode) {
        payload = {
          ...payload,
          no: initialData?.no,
      ...
      const res = await fetch('/api/advance', {
        method: 'POST', // Selalu POST untuk Apps Script
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const result = await res.json();
      if (result.status === 'success') {
        toast.success('Data advance berhasil dihapus!');
        onSuccess();
        onClose();
      } else {
