...
import {
  CoCrCeramicFemoralHeadLearningData,
  LearningSection,
  Language,
} from "@/components/operasi/hip/data/coCrCeramicFemoralHeadLearningData";
...
export default function FemoralHeadInteractiveLearning() {
  ...
  const sections = useMemo<LearningSection[]>(
    () =>
      [...CoCrCeramicFemoralHeadLearningData].sort(
        (a, b) => a.order - b.order
      ),
    []
  );
    ...
    <section className="max-w-5xl mx-auto px-4 py-14 space-y-10">
      ...
      <div className="text-center space-y-3">
        <Badge className="mx-auto flex w-fit items-center gap-2">
          <Circle size={14} />
          Femoral Head Surgical Learning
        </Badge>

        <h1 className="text-3xl font-bold tracking-tight">
          CoCr & Ceramic Femoral Heads
        </h1>

        <p className="text-sm text-muted-foreground max-w-xl mx-auto">
          {lang === "id"
            ? "Modul edukasi teknik bedah pemasangan kepala femoral berbahan Cobalt-Chromium dan Keramik."
            : "Educational module for surgical implantation of Cobalt-Chromium and Ceramic femoral heads."}
        </p>
      </div>
