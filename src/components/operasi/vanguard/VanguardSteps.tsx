...
const VanguardStepsGallery = () => {
  ...
  const nextImage = () => {
    setSelectedImage((prev) => (prev + 1) % step.image.length);
  };

  const prevImage = () => {
    setSelectedImage((prev) => (prev - 1 + step.image.length) % step.image.length);
  };
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 sm:p-6">
      ...
      <div className="flex flex-col md:flex-row gap-4">
        ...
        >
          ...
          <div className="relative w-full h-[350px] rounded overflow-hidden border">
            ...
            {step.image.length > 1 && (
              <>
                <button onClick={prevImage} className="absolute left-2 top-1/2 transform -translate-y-1/2 bg-white/80 px-2 py-1 rounded">←</button>
                <button onClick={nextImage} className="absolute right-2 top-1/2 transform -translate-y-1/2 bg-white/80 px-2 py-1 rounded">→</button>
              </>
            )}
          </div>
        </motion.div>
      ...
      <div className="mt-10">
        <h3 className="text-lg font-semibold mb-3 text-center md:text-left">Video Edukasi Vanguard:</h3>
